...
from ..core.constants import (
    ...
    IntentType,
    STATUS_OTP_REQUIRED,
    STATUS_OTP_INCORRECT,
    STATUS_ERROR,
    STATUS_SUCCESS,
    STATUS_FAILED,
    MAX_OTP_ATTEMPTS,
    MAX_INVALID_SELECTION_ATTEMPTS,
...
class TransferFlowHandler:
    ...
    def initiate_transfer(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Extracting transfer details from user input: %s", user_input)
            ...
            logger.info(
                f"Extracted details - Amount: {amount}, To: {to_beneficiary_name}, From: {from_account}, Frequency: {frequency}"
            )
            ...
            logger.info(f"Beneficiary resolution result: {beneficiary_result}")
            ...
            if (
                ...
                logger.info(
                    "Multiple beneficiaries found for '%s', transitioning to BENEFICIARY_SELECTION",
                    to_beneficiary_name,
                )
                return state
            ...
            if from_account is None:
                account_options = transfer_tool.load_accounts(user_id)
                # from_account NOT specified - ask user to select
                state["phase"] = ConversationPhase.ACCOUNT_SELECTION
                logger.info(
                    "From_account not specified, prompting user to select account."
                )
                ...
                logger.info(
                    "Single found, from_account not specified, transitioning to ACCOUNT_SELECTION"
                )
                logger.info(f"Pending transfer details: {state['pending_transfer']}")
                state["selection_attempts"] = 0
                state["result"] = {
                    STATUS_SUCCESS: True,
                ...
                logger.info(state)
                logger.info(
                    "Single beneficiary found, from_account not specified, transitioning to ACCOUNT_SELECTION"
                )
                return state
            else:
                ...
                logger.info(
                    "Single beneficiary found, from_account specified, transitioning to TRANSFER_SUMMARY"
                )
                return state
    ...
    def handle_beneficiary_selection(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not selected:
            ...
            if attempts >= MAX_INVALID_SELECTION_ATTEMPTS:
                logger.warning(
                    "Max beneficiary selection attempts exceeded for user %s",
                    state.get("user_id"),
                )
                state["result"] = {
                    STATUS_ERROR: True,
        ...
        logger.info("Beneficiary '%s' selected by user", selected.get("name"))
        logger.info(f"Pending transfer before beneficiary selection: {pending}")
        logger.info(f"From_account in pending transfer: {from_account}")
        ...
        if from_account is None:
            ...
            logger.info(
                "Beneficiary selected: %s, from_account not specified, moving to ACCOUNT_SELECTION",
                selected.get("name"),
            )
            return state
        else:
            ...
            logger.info(
                "Beneficiary selected: %s, from_account specified, moving to TRANSFER_SUMMARY",
                selected.get("name"),
            )
            return state
    ...
    def handle_account_selection(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        StateManager.ensure_defaults(state)
        logger.info("Handling account selection phase")
        ...
        if user_account not in ["savings", "current"]:
            ...
            if attempts >= MAX_INVALID_SELECTION_ATTEMPTS:
                logger.warning(
                    "Max account selection attempts exceeded for user %s",
                    state.get("user_id"),
                )
                state["result"] = {
                    STATUS_ERROR: True,
        ...
        logger.info(
            "Account selected: %s, moving to TRANSFER_SUMMARY",
            user_account.capitalize(),
        )
        return state
    ...
    def handle_transfer_summary_confirmation(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        StateManager.ensure_defaults(state)
        logger.info("Handling transfer summary confirmation phase")
        logger.info(f"Current state: {state}")
        ...
        if user_input not in ("yes", "no"):
            ...
            if attempts >= MAX_INVALID_SELECTION_ATTEMPTS:
                logger.warning(
                    "Max confirmation attempts exceeded for user %s",
                    state.get("user_id"),
                )
                state["result"] = {
                    STATUS_ERROR: True,
        ...
        if user_input == "no":
            # User cancelled
            state["result"] = {STATUS_FAILED: True, "message": "Transfer cancelled."}
            state["phase"] = ConversationPhase.NORMAL
            state["pending_transfer"] = None
            state["intent"] = IntentType.UNKNOWN
            return state
