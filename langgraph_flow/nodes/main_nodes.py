...
Spend, FAQ, Offers, Voice, and Unknown intent handlers.
Each enriched with contextual questions and language translation.
"""
...
from ..handlers.voice_node import handle_voice_interaction
from ..handlers.contextual_questions_node import handle_contextual_questions_node
from ..handlers.language_translation_node import translate_response_to_user_language
...
def spend_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Handle spend/transaction insights with language translation."""
    StateManager.ensure_defaults(state)
    user_id = state.get("user_id", "0")
    # Use translated English input for processing
    user_input = state.get("user_input_en", state.get("user_input", ""))
    ...
    state = translate_response_to_user_language(state)
    
    return state


def faq_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Handle FAQ/help queries with language translation."""
    StateManager.ensure_defaults(state)
    user_id = state.get("user_id", "0")
    # Use translated English input for processing
    user_input = state.get("user_input_en", state.get("user_input", ""))
...
def offers_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Handle offers/promotions queries with language translation."""
    StateManager.ensure_defaults(state)
    user_id = state.get("user_id", "0")
    # Use translated English input for processing
    user_input = state.get("user_input_en", state.get("user_input", ""))
...
def voice_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Handle voice interaction with language translation."""
    StateManager.ensure_defaults(state)
    user_id = state.get("user_id", "1")
    # Use translated English input for processing
    user_input = state.get("user_input_en", state.get("user_input", ""))
...
def unknown_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Handle unknown intents with language translation."""
    StateManager.ensure_defaults(state)
